...
function nFactorial(n, count = n, result = 1) {
  if (count == 0) return result;
  result = result * n;
  count -= 1;
  n -= 1;
  return nFactorial(n, count, result);
}

function nFibonacci(n, count = 0, result = [0, 1]) {
  if (count === n) return result[n];
  count++;
  result.push(result[result.length - 2] + result[result.length - 1]);
  return nFibonacci(n, count, result);
}
...
Queue.prototype.size = function (...args) {
